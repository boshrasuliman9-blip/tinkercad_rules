...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (currentUser && currentUser.role !== 'teacher') localStorage.removeItem('cq_session');
  ...
  if (reason === 'teacher_login_required')  { msg.textContent = 'يلزمك تسجيل الدخول بحساب معلم أولاً.'; msg.className = 'auth-msg error'; }
  else if (reason === 'teacher_pending')    { msg.textContent = 'حساب المعلم بانتظار موافقة الأدمن.'; msg.className = 'auth-msg error'; }
  else if (reason === 'teacher_rejected')   { msg.textContent = 'تم رفض طلب المعلم. راجع الأدمن لإعادة التقديم.'; msg.className = 'auth-msg error'; }
});
