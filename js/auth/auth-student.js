...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (user && user.role === 'student') { window.location.href = Auth.getRedirectPage(user); return; }
  if (user && user.role !== 'student') localStorage.removeItem('cq_session');
  ...
  if (reason === 'login_required_for_tracks') {
    const msg = document.getElementById('msg-signin');
    msg.textContent = 'سجّل الدخول أولاً حتى تدخل صفحة المسارات.'; msg.className = 'auth-msg error';
  }
});
